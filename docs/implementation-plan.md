...
- iTerm2 placement first uses the native current-window query. Creation and the single write then run inside one fixed handler loaded from the Launcher's precompiled AppleScript resource. The generated command is passed as one text descriptor argument, not source text; the returned object never crosses into Swift. The handler omits iTerm2's optional creation `command`, requires explicit Boolean true, and never retries or guesses from `current window` after a failure.
...
The latest clean Unit suite passes all 224 top-level tests and all 253 expanded device/configuration cases with zero failures or skips. This includes production Finder Workspace resolver/resource-inspector cases, production Finder Toolbar identity/signing inspector cases, platform-context and read-only status-service cases, Launcher runtime glue, the virtual Finder view classification that now produces a user-facing non-folder result instead of a malformed-reply diagnostic, and Terminal.app warm/cold/process-exit-race Handoff regressions. Preference, modifier, readiness, picker, terminal, and lifecycle regressions remain covered. App identity tests assemble isolated temporary outer and nested fixtures from the test Mach-O and ad-hoc sign those fixtures; the Unit scheme does not build, register, install, or launch a Debug or Release App. These tests do not send a real Finder or terminal Apple Event, modify Finder, present UI, or exercise TCC.
