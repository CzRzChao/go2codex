...
7. **iTerm2 + Codex CLI**：验证没有现有窗口时新建窗口，以及有现有窗口时按设置新建标签或窗口且不改变原标签；连续执行五次。新会话必须进入测试文件夹，只提交固定 `codex` 命令。
8. **iTerm2 + Claude Code CLI**：重复新标签、新窗口和无现有窗口路径，只提交固定 `claude` 命令。
