...
- **Fail-closed terminal targeting.** Terminal New Tab uses Terminal's public folder service instead of keyboard simulation. A mode-`0600` advisory lock serializes Go2Codex tab transactions across Launcher processes. Before and after the service call, Go2Codex reads coherent snapshots of all Terminal window IDs, tab counts, and TTY readiness; it submits only after consecutive snapshots preserve every prior TTY and contain exactly one new TTY. Missing, malformed, timed-out, observably concurrent, or ambiguous state never falls back to an untargeted command and is never retried automatically. Terminal exposes TTYs but no stable tab IDs, so an external close/open operation with immediate TTY reuse cannot be distinguished in principle; users should not manually modify Terminal tabs during the brief handoff. iTerm creation also has no automatic retry: an execution result that could have been lost after iTerm accepted the request is reported as outcome-unknown so the user can check for an existing session before trying again.
