...
if [[ "$content_contract" == "current" ]]; then
    assert_equal "$(plist_key_count "$app_path/Contents/Resources/zh-Hans.lproj/Localizable.strings")" "117" "outer Simplified Chinese string count"
    assert_equal "$(plist_key_count "$inner_path/Contents/Resources/zh-Hans.lproj/Localizable.strings")" "117" "Launcher Simplified Chinese string count"
fi
for strings_file in \
    ...
    if [[ "$content_contract" == "current" ]]; then
        ...
        assert_equal "$(plist_value "$strings_file" "Cursor opens the current Finder folder in a project (IDE) window. Cursor Agents Window is not supported.")" "Cursor 会在项目（IDE）窗口中打开 Finder 当前文件夹；不支持 Cursor Agents Window。" "Cursor project window guidance localization"
        ...
        assert_equal "$(plist_value "$strings_file" "Another Terminal handoff is already in progress")" "另一个 Terminal 交接正在进行" "Terminal handoff lock localization"
        assert_equal "$(plist_value "$strings_file" "Go2Codex could not safely inspect Terminal tabs")" "Go2Codex 无法安全检查 Terminal 标签页" "Terminal snapshot localization"
        ...
        assert_equal "$(plist_value "$strings_file" "Go2Codex could not start the CLI in the Terminal window")" "Go2Codex 无法在 Terminal 窗口中启动 CLI" "Terminal window CLI start localization"
        assert_equal "$(plist_value "$strings_file" "Wait for the current Terminal handoff to finish, then try again.")" "请等待当前 Terminal 交接完成，然后重试。" "Terminal handoff lock guidance localization"
        ...
        assert_equal "$(plist_value "$strings_file" "The Terminal window was created, but its TTY did not become ready, so no command was submitted. Close the empty window and try again. Go2Codex did not retry automatically.")" "Terminal 窗口已创建，但其 TTY 未就绪，因此未提交任何命令。请关闭空窗口后重试。Go2Codex 未自动重试。" "Terminal window TTY readiness localization"
        assert_equal "$(plist_value "$strings_file" "A Terminal window was created, but Go2Codex could not safely identify it, so no command was submitted. Close the empty window and try again. Go2Codex did not retry automatically.")" "Terminal 窗口已创建，但 Go2Codex 无法安全识别它，因此未提交任何命令。请关闭空窗口后重试。Go2Codex 未自动重试。" "Terminal window identity ambiguity localization"
        assert_equal "$(plist_value "$strings_file" "No Terminal tab was requested and no command was submitted. Wait for existing Terminal tabs to finish opening, then try again.")" "未请求创建 Terminal 标签页，也未提交任何命令。请等待现有 Terminal 标签页完成打开，然后重试。" "Terminal baseline localization"
        assert_equal "$(plist_value "$strings_file" "No command was submitted. Check Terminal for an empty tab before trying again, or choose New Window in Go2Codex Settings.")" "未提交任何命令。重试前请先检查 Terminal 中是否有空标签页，或在 Go2Codex 设置中选择“新窗口”。" "Terminal no-submission localization"
        assert_equal "$(plist_value "$strings_file" "No command was submitted. Go2Codex did not retry automatically. Check Terminal for an empty window before trying again.")" "未提交任何命令。Go2Codex 未自动重试。重试前请检查 Terminal 中是否有空窗口。" "Terminal window no-submission localization"
        assert_equal "$(plist_value "$strings_file" "No command was submitted. Go2Codex did not retry automatically. Try again.")" "未提交任何命令。Go2Codex 未自动重试。请重试。" "Terminal handoff no-submission localization"
        assert_plist_key_absent "$strings_file" "Another Terminal tab handoff is already in progress" "obsolete tab-specific Terminal handoff title"
        assert_plist_key_absent "$strings_file" "Wait for the current Terminal tab handoff to finish, then try again." "obsolete tab-specific Terminal handoff guidance"
        ...
        assert_plist_key_absent "$strings_file" "Accessibility permission is required for Terminal tabs" "obsolete Accessibility title localization"
        assert_plist_key_absent "$strings_file" "Automation permission is required for Terminal tabs" "obsolete Terminal tab Automation title localization"
        assert_plist_key_absent "$strings_file" "Open Accessibility Settings" "obsolete Accessibility localization"
        assert_plist_key_absent "$strings_file" "Locate Current Launcher" "obsolete Launcher recovery localization"
        assert_plist_key_absent "$strings_file" "Current Launcher could not be located" "obsolete Launcher recovery failure localization"
        assert_plist_key_absent "$strings_file" "Current Launcher is selected in Finder" "obsolete Launcher recovery success localization"
        assert_plist_key_absent "$strings_file" "Keep the Finder window open. In Accessibility, remove any older Go2CodexLauncher entry, then add the selected launcher and turn it on." "obsolete Accessibility recovery localization"
        assert_plist_key_absent "$strings_file" "Open Go2Codex.app in Finder, choose Show Package Contents, then open Contents > Helpers to find Go2CodexLauncher.app." "obsolete Launcher package recovery localization"
        assert_plist_key_absent "$strings_file" "Open System Settings, choose Privacy & Security, then enable Go2Codex in the requested Automation or Accessibility section." "obsolete mixed permission fallback localization"
        assert_plist_key_absent "$strings_file" "Terminal New Tab needs the current Go2CodexLauncher in Accessibility. If an older Go2CodexLauncher entry exists, remove it. Then choose Locate Current Launcher, add the revealed launcher to Accessibility, turn it on, and try again. Unsigned preview updates can invalidate an earlier entry." "obsolete Accessibility recovery guidance localization"
        assert_plist_key_absent "$strings_file" "Allow Go2Codex to control System Events in System Settings > Privacy & Security > Automation, then try again." "obsolete System Events localization"
    fi
done
