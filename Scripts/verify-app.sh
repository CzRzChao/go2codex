...
for strings_file in \
    ...
    if [[ "$content_contract" == "current" ]]; then
        ...
        assert_equal "$(plist_value "$strings_file" "Another Terminal tab handoff is already in progress")" "另一个 Terminal 标签页交接正在进行" "Terminal handoff lock localization"
        ...
        assert_equal "$(plist_value "$strings_file" "Wait for the current Terminal tab handoff to finish, then try again.")" "请等待当前 Terminal 标签页交接完成，然后重试。" "Terminal handoff lock guidance localization"
