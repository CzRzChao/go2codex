...
- **New Window and New Tab** are supported for both CLI targets in Terminal.app and iTerm2. Terminal New Tab invokes Terminal's public **New Terminal Tab at Folder** service with the Workspace directory; it does not use System Events, simulated keyboard input, or Accessibility. Go2Codex serializes its own tab-creation transactions, compares stable snapshots of every Terminal window and TTY, and submits the command exactly once only after the global tab count increases by one and the single new TTY is confirmed in consecutive snapshots. Terminal Automation is still required to inspect tabs and submit the command. A failed service, unstable or ambiguous state, an observed concurrent tab change, or a TTY that never becomes ready stops without guessing or retrying; an empty tab can remain when Terminal created one but Go2Codex could not safely target it. Terminal exposes no stable tab ID, so avoid manually opening or closing Terminal tabs during the brief handoff.
